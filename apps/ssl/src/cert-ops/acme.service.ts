...
export class CertService {
    ...
    async createAccount(createAccountDto: CreateAccountDto) {
        ...
        try {
            ...
            const identifiers = await Promise.all(acmeOrder.identifiers.map(async (identifier) => {
                ...
                if (auth) {
                    ...
                    if (dnsChallenge) {
                        const dns = await client.getChallengeKeyAuthorization(dnsChallenge);
                        return {
    ...
    async verifyDns01Challenge(id: number) {
        ...
        try {
            ...
            return {
                ...
                message: isDnsVerified ? 'dns-01验证成功,正在为您准备证书' : 'dns验证未通过'
            }
        } catch (error) {
    ...
    async deleteAccount(accountId: number) {
        ...
        try {
            ...
            if (!account) {
                throw new BusinessException(`ACME Account with ID ${accountId} not found`);
            }
            ...
            await queryRunner.rollbackTransaction();
            throw new BusinessException(`Failed to delete ACME Account: ${error.message}`);
        } finally {
            await queryRunner.release();
        }
