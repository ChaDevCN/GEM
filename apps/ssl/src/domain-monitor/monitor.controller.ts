...
export class CertificateMonitoringController {
    constructor(private readonly certificateService: CertificateMonitoringService) { }

    @Post()
    async create(@Body() createDto: CreateCertificateMonitoringDto, @Request() req): Promise<CertificateMonitoring> {
        try {
    ...
    @Get(':id')
    async findOne(@Param('id', ParseIntPipe) id: number): Promise<CertificateMonitoring> {
    ...
    async updateCheckInterval(
        ...
        const updatedCertificate = await this.certificateService.updateCheckInterval(id, updateDto.newInterval);
