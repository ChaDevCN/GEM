...
const Index = ({
	options: { open, setOpen, loading = false, type, run, data, closeCallback }
}: Props) => {
	const onClose = () => {
	...
	const items: DescriptionsProps['items'] = [
		{
			...
			label: '状态',
			children: (
				<Tag color={statusMap[data?.status || 'invalid'].color}>
					{statusMap[data?.status || 'invalid'].text}
				</Tag>
			)
		},
		{
		...
		},
		{
			...
			children: (
				<div>
					...
					<CopyToClipboard
						text={`_acme-challenge.${data?.value.split('.')[0]}` || '复制失败'}
	...
	return (
		...
		>
			<Spin spinning={loading}>
				{type === 'add' && (
					<ProForm onFinish={run} disabled={loading}>
						<ProFormGroup>
							<ProFormText
								width="md"
								name="domain"
