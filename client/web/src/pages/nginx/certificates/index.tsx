...
const Page = () => {
	...
	// delete
	const { run: deleteAccount, loading: deleteLoading } = useRequest(
		(id) => deleteAcmeAccount(id),
		{
			manual: true,
			onSuccess: ({ status, message: msg }) => {
				if (status === 200) {
	...
	const {
		run: runDns,
		cancel,
		loading: chanllengeLoading
	} = useRequest((id) => verifyDnsChallenge<{ status: string }>(id), {
		manual: true,
	...
	const onClick = (item: AccoutItem, key: string) => {
		switch (key) {
			...
			case '3':
				if (
					item.orders[0].identifiers[0].privateKey &&
					item.orders[0].identifiers[0].certificate
				) {
					downloadZip({
	...
	const columns: ProColumns<AccoutItem>[] = [
		...
		},
		{
			...
			render: (_, item) => {
				...
				return (
					<Dropdown
						menu={{ items, onClick: ({ key }) => onClick(item, key) }}
						placement="bottom"
						arrow
					>
	...
	return (
		<Card>
			...
			<ProTable
				actionRef={tableRef}
				request={async () => {
					const res = await getAccount<AccoutItem[]>();
					let { data } = res;
					const { status } = res;
					if (status !== 200) {
						data = [];
